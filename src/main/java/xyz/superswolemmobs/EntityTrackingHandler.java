...
public class EntityTrackingHandler {
    ...
    public void onPlayerClientsideTickHandleTracking(TickEvent.PlayerTickEvent e) {
        ...
        if (!LongDistanceTracking.allowCustomRenderDistance) return;
        ...
        for (Entity loadedEntity : currentWorldInstance.loadedEntityList) {
            ...
            loadedEntity.renderDistanceWeight = (double) LongDistanceTracking.customRenderDistanceWeight;
            modifiedEntities.add(loadedEntity);

            if (LongDistanceTracking.debugMode) {
                LongDistanceTracking.modLogger.log(Level.INFO,
